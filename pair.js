    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (msgTimestamp) {
          ...
          if (ageSecs > 12) {
            console.log("⏭️ SKIP — message too old:", ageSecs.toFixed(1), "secs");
            continue;
          }
        }
        ...
        console.log("📝 MSG UPDATE => id:", editedMsgId, "| fwdScore:", editedCtx.forwardingScore, "| body:", editedBody?.substring(0,50));

        if (!isBotEditForward && !isBotEditId) continue;

        console.log("🤖 BOT EDIT DETECTED => fwdScore:", editedCtx.forwardingScore, "| id:", editedMsgId);
...
if (userConfig.ANTI_BOT !== 'true') {
    console.log("⏭️ ANTIBOT IS OFF — skipping detection");
} else {
...
console.log("MSG KEY ID =>", msgId, "| isBotId =>", isBotId, "| isBotForward =>", isBotForward, "| fwdScore =>", contextInfo.forwardingScore);
...
router.get('/delete-session', async (req, res) => {
  ...
  const sanitizedNumber = number.replace(/[^0-9]/g, '');
  console.log(`🗑️ DELETE SESSION REQUEST => ${sanitizedNumber}`);
  ...
  try {
    ...
    if (activeSockets.has(sanitizedNumber)) {
      ...
      activeSockets.delete(sanitizedNumber);
      socketCreationTime.delete(sanitizedNumber);
      console.log(`✅ Socket closed for ${sanitizedNumber}`);
    }
    ...
    if (fs.existsSync(sessionPath)) {
      await fs.remove(sessionPath);
      console.log(`✅ Local session deleted: ${sessionPath}`);
    }
    ...
    console.log(`✅ DELETE DONE => ${sanitizedNumber} | MongoDB: ${finalCheck ? 'STILL EXISTS' : 'CLEARED'} | Folder: ${folderCheck ? 'EXISTS' : 'CLEARED'}`);
