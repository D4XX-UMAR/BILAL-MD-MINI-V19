...
async function handleManualUnlink(number) {
  ...
  try {
    ...
    console.log(`🧹 MongoDB delete requested for: ${sanitizedNumber}`);

const checkSession = await getSessionFromMongoDB(sanitizedNumber);

if (checkSession) {
    console.log(`❌ DEBUG: Session STILL EXISTS in MongoDB for ${sanitizedNumber}`);
    console.log(`📦 DEBUG DATA:`, JSON.stringify(checkSession, null, 2));
} else {
    console.log(`✅ DEBUG: Session successfully removed from MongoDB for ${sanitizedNumber}`);
}

console.log(`📂 Session folder exists: ${fs.existsSync(sessionPath)}`);
console.log(`🔌 Active socket exists: ${activeSockets.has(sanitizedNumber)}`);
console.log(`⏰ Socket time exists: ${socketCreationTime.has(sanitizedNumber)}`);
console.log("🌍 Global lock exists:", global[`connecting_${sanitizedNumber}`] ? true : false);
...
console.log(`🧹 FULL CLEANUP DONE => ${sanitizedNumber}`);
...
async function setupStatusHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
const userConfig =
global.runtimeConfig[number]
|| await getUserConfigFromMongoDB(number);
console.log("========== STATUS DEBUG ==========");
console.log("NUMBER:", number);
console.log("AUTO_VIEW_STATUS =", userConfig.AUTO_VIEW_STATUS);
console.log("==================================");
...
console.log("✅ STATUS READ SUCCESS");
break;
          } catch (error) {
            retries--;
...
global.runtimeConfig[number]
|| await getUserConfigFromMongoDB(number);
      console.log(`DEBUG: Current AntiCall status is: ${userConfig.ANTI_CALL}`);
...
async function POPKIDMDPair(number, res) {
  ...
  try {
    ...
    if (!existingSession) {
      ...
      if (!restoredCreds) {
   console.log(`🆕 No valid session found, generating fresh pair code`);
}
      if (restoredCreds) {
...
console.log("COMMANDS AFTER LOAD =>", events.commands.length);
...
console.log("FINAL COMMANDS =>", events.commands.length);
console.log("FIRST CMD =>", events.commands[0]);
...
async function setupPOPKIDCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    console.log("========== COMMAND SCAN START ==========");
...
console.log("FIRST COMMAND =>", events.commands?.[0]);
    try {
        for (const cmdData of events.commands) {

            console.log("CHECKING CMD =>", cmdData.pattern);
    ...
    detectedCommand = body
      ...
      .toLowerCase();
        ...
        console.log("❌ COMMAND SCAN ERROR =>", err);
    }
}
...
if (
    ...
    console.log("🤖 POSSIBLE BOT REPLY =>", body);
}
console.log("========== COMMAND SCAN END ==========");
...
console.log("🤖 BAILEYS MESSAGE DETECTED");
...
console.log("IS BAILEYS =>", m.isBaileys);
console.log("MESSAGE OBJECT =>", JSON.stringify(msg.message, null, 2));

console.log("========== ANTIBOT DEBUG ==========");
console.log("GROUP =>", from);
console.log("SENDER =>", senderNumber);
console.log("MSG ID =>", msg.key?.id);
console.log("IS BAILEYS =>", m.isBaileys);
console.log("BOT NUMBER =>", botNumber);
console.log("FROM_ME =>", msg.key?.fromMe);
console.log("===================================");

console.log("========== ANTIBOT HANDLER START ==========");

console.log("🤖 SCANNING FOR EXTERNAL BOTS (ULTIMATE MODE)...");

    const action = userConfig.ANTI_BOT_ACTION || "kick";
    
const isExternalBot = (m.isBaileys || msg.key.id.length > 25 || msg.key.id.length < 15 || msg.key.id.startsWith('3EB0') || msg.key.id.startsWith('BAE5') || msg.key.id.startsWith('3A') || msg.key.id.startsWith('3E') || msg.key.id.startsWith('AC') || msg.key.id.startsWith('A5') || msg.key.id.startsWith('B1E')) && !msg.key.fromMe && !(body && body.includes(BOT_TOKEN));        

        if (isExternalBot) {
            console.log("🚀 EXTERNAL BOT DETECTED! ID:", msg.key.id, "| ACTION =>", action);

            // 1. DELETE ACTION
            if (action === "delete") {
                try {
                    await socket.sendMessage(from, { delete: msg.key });
                    await reply("🤖 *Bot Detected:* Unka message delete kar diya gaya hai. 🗑️");
            ...
            // 2. KICK ACTION
                ...
                try {
                    await reply("🚫 *Bot Detected:* Removing this external bot from group...");
                    await socket.groupParticipantsUpdate(from, [nowsender], "remove");
                } catch (err) { console.log("❌ KICK ERROR =>", err); }
            }

            // 3. WARN ACTION
            else if (action === "warn") {
                try {
                    await reply("⚠️ *Warning:* External bots are not allowed in this group! Agli baar kick kar diya jayega. 😤");
                } catch (err) { console.log("❌ WARN ERROR =>", err); }
            }

            console.log("========== ANTIBOT HANDLER END ==========");
    ...
    console.log("✅ NORMAL USER OR OUR BOT - CONTINUING TO COMMANDS");
    // --- MONGODB BAN SYSTEM CHECK ---
    const isBannedUser = await BannedUser.findOne({
        number: senderNumber
    });
    console.log("🔍 BAN DATABASE RESULT =>", isBannedUser);
  ...
  return await socket.sendMessage(sender, { text: teks + BOT_TOKEN }, { quoted: myquoted });
};
    
    if (isBannedUser && !isOwner) {
    console.log("USER BLOCKED =>", senderNumber);
...
STATUS: ${userConfig.ANTI_BOT === 'true' ? 'ON ✅' : 'OFF ❌'}
...
if (!args[1]) {
return reply(`🤖 ANTIBOT ACTION

Current: ${userConfig.ANTI_BOT_ACTION || "kick"}
...
Bot messages auto delete honge.`);
}
...
Bot detect hone par warning milegi.`);
}
...
Bot detect hote hi remove hoga.`);
}
}
}
...
target = m.quoted.chat;

console.log("TARGET FROM QUOTED =>", target);
...
USE :❯ BAN
REPLY YA MENTION KISI USER KO KARO
...
*YA GROUPS ME INBOX ME KISI K MSG KO MENTION KR KE LIKHO BAN TAB BHI WORK KRE GA*

*JAB USER BAN HO GAYA TO WO BOT KA KOI BHI COMMAND USE NAHI KR SKTA 😂*`);
}

const targetNumber = target.replace(/@.+/, "");
console.log("TARGET NUMBER =>", targetNumber);

await BannedUser.create({
number: targetNumber
});
console.log("BAN SAVED =>", targetNumber);
return reply(`*👑 USER BANNED SUCCESSED 👑*
...
if (num.length < 8) {
return reply("*NUMBER SAHI LIKHO 😊*");
}
...
if (!target) {
return reply("*👑 UNBAN SYSTEM INFO 👑*\n*YA NUMBER DO*\n\n*EXAMPLE :❯*\n*UNBAN*\n*UNBAN @USER*\n*UNBAN 923xxxxxx*\n\n*YA GROUPS ME INBOX ME KISI K MSG KO MENTION KR KE LIKHO UNBAN TAB BHI WORK KRE GA*\n\n*JAB USER UNBAN HO GAYA TO WO BOT KA ALL COMMANDS USE KAR SKTA HA 😊*\n\n*👑 BILAL-MD WHATSAPP BOT 👑*");
}
...
console.log("UNBAN TARGET =>", target);
console.log("UNBAN NUMBER =>", targetNumber);
console.log("UNBAN DB RESULT =>", banned);

if (!banned) {
return reply("*YEH USER BANNED NAHI HAI 🙂*");
}
...
if (args[0] === "on") {
userConfig.AUTO_VIEW_STATUS = "true";
console.log("🟢 COMMAND SET AUTO_VIEW_STATUS =", userConfig.AUTO_VIEW_STATUS);
global.runtimeConfig[number] = userConfig;
await updateUserConfigInMongoDB(number, userConfig);
