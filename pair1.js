...
const userConfig = await getCachedConfig(number);
    // ✅ FIX 1: ALWAYS_OFFLINE on hai to recording presence bilkul mat bhejo
    if (userConfig.ALWAYS_OFFLINE === 'true') return;
...
function setupAutoRestart(socket, number) {
 ...
 if (isManualUnlink || errorMessage?.includes('401')) {
    ...
    if (connection === 'open') {
      restartAttempts = 0;
      // 🔥 GLOBAL GHOST HEARTBEAT (Status Freeze System)
      setInterval(async () => {
          try {
              const liveConfig = await getCachedConfig(number);
              if (liveConfig.ALWAYS_OFFLINE === 'true') {
                  await socket.sendPresenceUpdate('unavailable');
              } else if (liveConfig.ALWAYS_ONLINE === 'true') {
                  await socket.sendPresenceUpdate('available');
              }
          } catch (e) {}
      }, 3000); // ✅ FIX 3: 10s se 3s - gap kam, offline zyada tight
    }
  });
}
...
// 🛡️ THE SUPREME GHOST PROTECTOR (Anti-Leak System)
      
      // 1. Hook Presence: Kisi bhi typing/recording/available signal ko block karna
      const originalPresenceUpdate = socket.sendPresenceUpdate;
      socket.sendPresenceUpdate = async (presence, jid) => {
          const liveConfig = await getCachedConfig(sanitizedNumber);
          if (liveConfig.ALWAYS_OFFLINE === 'true') {
              // ✅ FIX 2: jid ho ya na ho, safely unavailable bhejo
              const args = jid ? ['unavailable', jid] : ['unavailable'];
              return originalPresenceUpdate.apply(socket, args).catch(() => {});
          }
          return originalPresenceUpdate.apply(socket, [presence, jid]);
      };

      // 2. Hook SendMessage: Reply dete waqt jo online leak hota hai usey rokna
      const originalSendMessage = socket.sendMessage;
      socket.sendMessage = async (...args) => {
          const liveConfig = await getCachedConfig(sanitizedNumber);
          if (liveConfig.ALWAYS_OFFLINE === 'true') {
              // Message bhejne se pehle offline thappa
              await originalPresenceUpdate.apply(socket, ['unavailable', args[0]]).catch(() => {});
          }
          
          const result = await originalSendMessage.apply(socket, args);

          if (liveConfig.ALWAYS_OFFLINE === 'true') {
              // Message bhejne ke foran baad offline thappa
              await originalPresenceUpdate.apply(socket, ['unavailable', args[0]]).catch(() => {});
          }
          return result;
      };

      // 3. Hook ReadMessages: Status dekhne par jo online leak hota hai usey rokna
      const originalReadMessages = socket.readMessages;
      socket.readMessages = async (keys) => {
          const result = await originalReadMessages.apply(socket, [keys]);
          const liveConfig = await getCachedConfig(sanitizedNumber);
          if (liveConfig.ALWAYS_OFFLINE === 'true') {
              await originalPresenceUpdate.apply(socket, ['unavailable']).catch(() => {});
          }
          return result;
      };

      // 4. Aggressive Heartbeat Loop (5 Second Freeze)
      setInterval(async () => {
          try {
              const liveConfig = await getCachedConfig(sanitizedNumber);
              if (liveConfig.ALWAYS_OFFLINE === 'true') {
...
var from = msg.key.remoteJid;
    var isGroup = from && from.endsWith("@g.us");
    const m = sms(socket, msg);
    
    // 🚀 HARDCORE GHOST ENFORCER (Immediate Response)
    // Ye message aate hi 0.001 second mein server ka mun band kar dega
    if (userConfig && userConfig.ALWAYS_OFFLINE === 'true') {
        // 1. Pehla thappa (Immediate)
        socket.sendPresenceUpdate('unavailable').catch(() => {});
        
        // 2. Dusra thappa (0.5s baad - Safety for auto-read/typing)
        setTimeout(() => {
            socket.sendPresenceUpdate('unavailable').catch(() => {});
        }, 500);
    } else if (userConfig && userConfig.ALWAYS_ONLINE === 'true') {
        socket.sendPresenceUpdate('available').catch(() => {});
    }

    // 🚀 GLOBAL BAILEYS LID-TO-JID PATCH
    if (isGroup && (m.sender || (m.quoted && m.quoted.participant))) {
        try {
            ...
            if (groupMetadata && groupMetadata.participants) {
                ...
                if (m.sender && m.sender.endsWith('@lid')) {
                    const user = parts.find(p => p.lid === m.sender || p.id === m.sender);
                    if (user) m.sender = user.id || user.phoneNumber;
                }

                if (m.quoted && m.quoted.participant && m.quoted.participant.endsWith('@lid')) {
                    const user = parts.find(p => p.lid === m.quoted.participant || p.id === m.quoted.participant);
                    if (user) m.quoted.participant = user.id || user.phoneNumber;
                }
                
                if (m.quoted && m.quoted.sender && m.quoted.sender.endsWith('@lid')) {
                    const user = parts.find(p => p.lid === m.quoted.sender || p.id === m.quoted.sender);
                    if (user) m.quoted.sender = user.id || user.phoneNumber;
                }
            }
        } catch (err) {
            console.log("Global LID Patch Error: ", err.message);
        }
    }
    // 🚀 PATCH END
...
if (command === "online") {
    ...
    if (!args[0]) {
        const status = userConfig.ALWAYS_ONLINE === 'true' ? 'ON' : 'OFF';
        return reply(`*👑 ALWAYS ONLINE INFO 👑*\n\n*TYPE :❯ ONLINE ON*\n*TYPE :❯ ONLINE OFF*\n\n*ABHI STATUS ❮ ${status} ❯ HAI*`);
    }

    const mode = args[0].toLowerCase();

    if (mode === "on") {
        // Conflict Check: Agar Offline pehle se ON hai
        if (userConfig.ALWAYS_OFFLINE === 'true') {
            return reply("*❌ ERROR: PEHLE OFFLINE MODE BAND KAREIN! 👻*\n\n*AP NE OFFLINE MODE ON KIA HUA HAI, AIK WAQT ME DONO CHAL NAHI SAKTE. PEHLE LIKHO: .OFFLINE OFF*");
        }
        
        await setCachedConfig(number, 'ALWAYS_ONLINE', 'true');
        userConfig.ALWAYS_ONLINE = 'true';
        await socket.sendPresenceUpdate('available');
        return reply("*👑 ALWAYS ONLINE ACTIVATED ✅*\n\n*AB BOT HAMESHA ONLINE DIKHEGA 😎*");
    }

    if (mode === "off") {
        await setCachedConfig(number, 'ALWAYS_ONLINE', 'false');
        userConfig.ALWAYS_ONLINE = 'false';
        await socket.sendPresenceUpdate('unavailable'); // Resetting status
        return reply("*👑 ALWAYS ONLINE DE-ACTIVATED ❌*");
    }
}
...
if (command === "offline") {
    ...
    if (!args[0]) {
        const status = userConfig.ALWAYS_OFFLINE === 'true' ? 'ON' : 'OFF';
        return reply(`*👑 ALWAYS OFFLINE INFO 👑*\n\n*TYPE :❯ OFFLINE ON*\n*TYPE :❯ OFFLINE OFF*\n\n*ABHI STATUS ❮ ${status} ❯ HAI*`);
    }

    const mode = args[0].toLowerCase();

    if (mode === "on") {
        // Conflict Check: Agar Online pehle se ON hai
        if (userConfig.ALWAYS_ONLINE === 'true') {
            return reply("*❌ ERROR: PEHLE ONLINE MODE BAND KAREIN! 🚀*\n\n*AP NE ALWAYS ONLINE ON KIA HUA HAI. PEHLE USE BAND KAREIN. LIKHO: .ONLINE OFF*");
        }

        await setCachedConfig(number, 'ALWAYS_OFFLINE', 'true');
        userConfig.ALWAYS_OFFLINE = 'true';
        await socket.sendPresenceUpdate('unavailable');
        return reply("*👑 ALWAYS OFFLINE ACTIVATED ✅*\n\n*AB BOT SAB KO OFFLINE DIKHEGA (GHOST MODE) 👻*");
    }

    if (mode === "off") {
        await setCachedConfig(number, 'ALWAYS_OFFLINE', 'false');
        userConfig.ALWAYS_OFFLINE = 'false';
        return reply("*👑 ALWAYS OFFLINE DE-ACTIVATED ❌*");
    }
}
