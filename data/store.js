...
// ============================================================
// 🚀 IN-MEMORY CACHE SYSTEM — File reads/writes bahut kam hongi
// ============================================================
const memoryCache = {};      // Data memory mein rakho
const writeTimers = {};      // Debounce timers
const WRITE_DELAY = 2000;    // 2 second baad file write hogi

// Memory cache se lo, nahi hai toh file se lo
const readJSON = async (file) => {
  if (memoryCache[file] !== undefined) {
    return memoryCache[file]; // ✅ Memory se instant return
  }
  try {
    const filePath = path.join(storeDir, file);
    const data = await fs.readFile(filePath, 'utf8');
    memoryCache[file] = JSON.parse(data);
...
// Pehle memory update karo, phir 2 sec baad file write karo
const writeJSON = async (file, data) => {
  memoryCache[file] = data; // ✅ Turant memory update

  // Agar pehle se timer chal raha hai, cancel karo
  if (writeTimers[file]) {
    clearTimeout(writeTimers[file]);
  }

  // 2 second baad file write karo (multiple writes batch ho jayengi)
  writeTimers[file] = setTimeout(async () => {
    try {
      ...
      console.error(`❌ Store write error [${file}]:`, err.message);
    }
...
const saveMessage = async (message) => {
  ...
  if (index > -1) {
    ...
    // Memory limit — sirf last 500 messages rakho
    if (messages.length >= 500) {
      messages.sort((a, b) => a.timestamp - b.timestamp);
      messages.splice(0, messages.length - 499);
    }
...
// codes by JawadTechX — optimized by BILAL-MD Cache System
